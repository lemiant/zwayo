function autocomplete() {
    ...
    if (search_query) {
        ...
        setTimeout(function () {
            if (my_query === window.search_query) {
        ...
        $('#search_message').text('Type a song to search').show();
    }
}
...
function get_thumb(row) {
    if (row.snippet.thumbnails.small) {
        return row.snippet.thumbnails.small.url;
    } else if (row.snippet.thumbnails.default) {
        return row.snippet.thumbnails.default.url;
    } else {
        return "";
    }
}
