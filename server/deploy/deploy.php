...
if (defined('AUTHOR') and $payload->commits[0]->author->name != AUTHOR){
    die("<h2>Only Alex has auto-deploy turned on</h2>");
}
...
if (defined('BACKUP_DIR') && BACKUP_DIR !== false && is_dir(BACKUP_DIR)) {
    $commands[] = sprintf(
        'tar czf %s%s-%s-%s.tar.gz %s*' //Slash must be included in dir path
        , BACKUP_DIR
        , basename(TARGET_DIR)
        , md5(TARGET_DIR)
        , date('YmdHis')
